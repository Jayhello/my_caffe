...
namespace caffe{
    ...
    void Timer::Start() {
        if (!running()) {
            if (Caffe::mode() == Caffe::GPU) {
                ...
                start_cpu_ = boost::date_time::microsec_clock<boost::posix_time::ptime>::local_time();
            }
            running_ = true;
    ...
    void Timer::Stop() {
        if (running()) {
            if (Caffe::mode() == Caffe::GPU) {
                ...
                stop_cpu_ = boost::date_time::microsec_clock<boost::posix_time::ptime>::local_time();
            }
            running_ = false;
        }
    }
    ...
    void CPUTimer::Start() {
        if (!running()) {
            this->start_cpu_ = boost::date_time::microsec_clock<boost::posix_time::ptime>::local_time();
            this->running_ = true;
    ...
    void CPUTimer::Stop() {
        if (running()) {
            this->stop_cpu_ = boost::date_time::microsec_clock<boost::posix_time::ptime>::local_time();
            this->running_ = false;
        }
    }
